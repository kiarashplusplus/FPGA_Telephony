...
//////////////////////////////////////////////////////////////////////////////
   ...
   defparam reset_sr.INIT = 16'hFFFF;
	...
	wire [20:0] tmt_cntr_DEBUG;
	CF_Interface CF_INTERFACE_1(
		.clk_27mhz(clk_27mhz),               // 27 MHz clock
		.reset(reset),                       // Synchronous reset
		// Mid-level I/O
		.cmd(cmd),                                 // Command to be performed
		.LBA(LBA),                                 // Logical Block Address for r/w
		.SC(SC),                                   // Sector Count
		.din(din),                                 // Data input for writes
		.we_req(we_req),                           // Request for write data
		.dout(dout),                               // Data output for reads
		.nd(nd),                                   // New Data available at output
		.CF_detect(CF_detect),                     // Detect if CF device is connected
		.LBA_max(LBA_max),                         // Maximum number of LBAs if CF detected
		.ready(ready),                             // Command ready signal
		// SystemACE ports
	...
	reg [5:0] state_old;
	always @(posedge clk_27mhz) state_old <= (reset) ? 0: state_DEBUG;
	wire state_en;
	assign state_en = (state_DEBUG != state_old)&(state_DEBUG != 6'h18)&(state_DEBUG != 6'h19)&(state_DEBUG != 6'h1A)&(state_DEBUG != 6'h1B)&(state_DEBUG != 6'h1C);
	...
	reg [11:0] cnt;
	always @(posedge clk_27mhz) cnt <= (reset) ? 0: (we_req) ? (cnt + 1): cnt;

	
	reg [11:0] rd_cnt;
	always @(posedge clk_27mhz) rd_cnt <= (reset) ? 0: (nd) ? (rd_cnt + 1): rd_cnt;
	...
	always @(posedge clk_27mhz) begin
		if (reset) begin
			...
			case (state)
				0: begin
					if (ready & b1_rise) begin
						cmd_reg <= 3;
						state <= state + 1;
						LBA_reg <= {8'h00, sw[3:0], sw, sw};
						SC_reg <= sw;
					end
				end
				1: begin
	...
	assign led[0] = ~CF_detect;
	assign led[1] = ~ready;
	assign led[2] = ~empty;
	assign led[3] = ~empty_2;
	assign led[4] = ~systemace_mpbrdy;
	assign led[5] = ~check;
	assign led[6] = 1'b1;
	assign led[7] = ~led7;

	assign data[63:56] = {3'h0, FIFO_dout};
	assign data[55:44] = cnt;
	assign data[43:32] = rd_cnt;
	assign data[31:0] = (sw[0]) ? {4'h0,LBA_max}: wr_cnt;
	
   assign analyzer1_data = systemace_data;
   assign analyzer1_clock = clk_27mhz;	
	assign analyzer3_data = {systemace_ce_b, systemace_we_b, systemace_oe_b,state_DEBUG,systemace_address[6:1],systemace_mpbrdy};
