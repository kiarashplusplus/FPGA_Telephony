...
////////////////////////////////////////////////////////////////////////////
  ...
  defparam reset_sr.INIT = 16'hFFFF;
	...
	wire [7:0] DLC_TX_1;
	reg DLC_TX_ready_1;
	wire [7:0] DLC_TX_addr_1;
	wire [7:0] DLC_RX_addr_1;
	wire [5:0] init_state_DEBUG;
	
	reg [7:0] self_phn_num;
	reg [7:0] other_phn_num;
	...
	MAIN_FIFO2 OUT_FIFO_2(
		.rd_clk(clk_27mhz),
		.wr_clk(clk_40mhz),
		.rst(reset),
		.din(DLC_RX_1),
		.wr_en(DLC_RX_ready_1),
		.dout(audio_out_data),
		.rd_en(audio_ready),
		.empty(out_empty)
		);
		...
		if (reset) begin
			...
			case (state)
				0: begin
					if (b_enter) begin
						state <= state + 1;
						self_phn_num <= {4'h0, sw[3:0]};
						other_phn_num <= {4'h0, sw[7:4]};
					end
				end
				1: begin
					cmd_1 <= 1;
				...
				4: begin
					if ((sts_1 == STS_IDLE) & (rd_data_count >= 6'd48)) begin
						state <= state + 1;
						cmd_1 <= 3;
					end
				end
				5: begin
					cmd_1 <= 0;
					if (sts_1 == STS_TX_ACCEPT) begin
						state <= state + 1;
						DLC_TX_ready_1 <= 1;
						cntr <= 6'd47;
					end
					else if (sts_1 == STS_TX_REJECT) begin
						state <= state - 1;
					end
				end
				6: begin
...
//	assign led[7] = ~tests_failed;	
	assign led[0] = ~link_sts_1;
	assign led[1] = ~IB_1;
	assign led[2] = ~in_full;
	assign led[3] = ~out_empty;
	assign led[4] = ~D_TX_ready_1;
	assign led[5] = ~D_RX_ready_1;
	assign led[7:6] = 5'hFF;
	
	assign data[15:0] = ~audio_in_data;
	assign data[31:16] = ~audio_out_data;
	assign data[39:32] = ~DLC_TX_1;
	assign data[47:40] = ~DLC_RX_1;
	assign data[51:48] = {1'b1, ~state};
	assign data[57:52] = ~init_state_DEBUG;
