...
//////////////////////////////////////////////////////////////////////////////
  ...
  defparam reset_sr.INIT = 16'hFFFF;
	...
	debounce DB1(.clock(clock_27mhz),.reset(reset),.noisy(~button_up),.clean(br_up));
	debounce DB2(.clock(clock_27mhz),.reset(reset),.noisy(~button_down),.clean(br_down));
	debounce DB3(.clock(clock_27mhz),.reset(reset),.noisy(~button_left),.clean(br_left));
	debounce DB4(.clock(clock_27mhz),.reset(reset),.noisy(~button_right),.clean(br_right));
	debounce DB5(.clock(clock_27mhz),.reset(reset),.noisy(~button_enter),.clean(br_enter));
	debounce DB6(.clock(clock_27mhz),.reset(reset),.noisy(~button0),.clean(br0));
	debounce DB7(.clock(clock_27mhz),.reset(reset),.noisy(~button1),.clean(br1));
	debounce DB8(.clock(clock_27mhz),.reset(reset),.noisy(~button2),.clean(br2));
	debounce DB9(.clock(clock_27mhz),.reset(reset),.noisy(~button3),.clean(br3));
	...
	Text_Scroller TS1(	
		...
		.reset(reset),
		// Data In
		.ascii_data(ascii_out),
		.ascii_data_ready(ascii_out_ready),
		// Data Out
		.string_data(string_data)
		);
	
	// Instantiate display string
	display_string DS1(
